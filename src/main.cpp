        ...
        if (client.connect("blockchain.info", 443)) {
            ...
            if (bodyStart > 0) {
                ...
                if (body.indexOf("error") > 0 || body.indexOf("invalid") > 0) {
                    strcpy(newBalance, "Invalid Addr");
                } else {
                    long satoshis = body.toInt();
                    float btc = satoshis / 100000000.0;
                    
                    if (satoshis == 0 && body != "0") {
                        strcpy(newBalance, "Check Addr");
                    } else if (btc > 0) {
                        sprintf(newBalance, "%.8f BTC", btc);
                    } else {
                        strcpy(newBalance, "0 BTC");
                    }
                }
                
                if (strcmp(newBalance, btcBalance) != 0) {
                    strcpy(btcBalance, newBalance);
                    displayDirty = true;
                }
            }
            client.stop();
...
// Optimized: Moved all display logic to a separate function
void drawDisplay(unsigned long elapsed, float cachedTemp, long local_hashes, long local_templates, int local_halfshares, int local_shares, int local_valids, bool local_blockFound, unsigned long local_blockTime) {
    ...
    if (local_blockFound && (millis() - local_blockTime < 60000)) {
    ...
    if (displayMode == 0) {
        ...
        M5.Lcd.setTextColor(YELLOW);
        M5.Lcd.setCursor(5, y);
        M5.Lcd.printf("Bal: %s", btcBalance);
        y += 16;
...
float cachedTemp = 0;
long local_hashes = 0;
long local_templates = 0;
int local_halfshares = 0;
int local_shares = 0;
int local_valids = 0;
bool local_blockFound = false;
unsigned long local_blockTime = 0;
...
void setup(){
    ...
    if (THREADS == 2) {
        ...
        xTaskCreatePinnedToCore([](void*){ miner1.start(); }, "M1", 35000, nullptr, 4, nullptr, 0);
        xTaskCreatePinnedToCore([](void*){ miner2.start(); }, "M2", 35000, nullptr, 4, nullptr, 1);
    }
    if (THREADS == 4) {
        static BitcoinMiner miner1("M1", 0);
        static BitcoinMiner miner2("M2", 1);
        ...
        xTaskCreatePinnedToCore([](void*){ miner1.start(); }, "M1", 35000, nullptr, 4, nullptr, 0);
        xTaskCreatePinnedToCore([](void*){ miner2.start(); }, "M2", 35000, nullptr, 4, nullptr, 1);
        xTaskCreatePinnedToCore([](void*){ miner3.start(); }, "M3", 35000, nullptr, 4, nullptr, 0);
        xTaskCreatePinnedToCore([](void*){ miner4.start(); }, "M4", 35000, nullptr, 4, nullptr, 1);
    }
...
void loop(){
    ...
    if (displayDirty) {
        ...
        local_halfshares = halfshares;
        local_shares = shares;
        local_valids = valids;
        local_blockFound = blockFound;
        local_blockTime = blockFoundTime;
        ...
        drawDisplay(elapsed, cachedTemp, local_hashes, local_templates, local_halfshares, local_shares, local_valids, local_blockFound, local_blockTime);
    }
